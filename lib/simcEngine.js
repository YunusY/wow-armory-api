...
async function runSimc(simcData, options = {}) {
    ...
    const htmlOutputPath = path.join(reportsDir, `${htmlId}.html`);
    ...
    try {
        ...
        console.log(`Starting SimC run: binary=${binary}, iterations=${iterations}, target_error=${targetError || '(none)'}, threads=${threads}, statistics_level=${statisticsLevel}, seed=${seed || '(random)'}`);
        ...
        return {
            reportId: htmlId,
            reportUrl: `/api/get-simc?report=${htmlId}`,
            iterations,
            totalDps,
