...
// logic. Bumped from 2 to 3 for: locked-in fight_style/max_time/etc. (old
// entries were simmed with different implicit settings) and the switch from
// a hardcoded MIN_PULL_DATE to auto-discovered cutoffs.
        ...
        for (const guildKey of Object.keys(TRACKED_GUILDS)) {
            ...
            try {
                ...
                g.history.push({
                    ...
                    fightStyle: null, maxTime: null, varyCombatLength: null, optimalRaid: null,
...
// `since`, if given, must already be a valid ISO datetime string (validated
// by the caller) — filters to pulls whose pullStartedAt is at or after it.
// `limit` caps how many (newest-first) entries come back after that filter.
function getGuildView(guildKey, { limit, since } = {}) {
    const g = state.guilds[guildKey];
    if (!g) {
        return {
